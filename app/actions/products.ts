 ...
 */
export async function applyGlobalRate(
  tasa: number,
): Promise<ActionResult<{ updated: number; products: Product[] }>> {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from(PRODUCTS_TABLE)
      .select("id, nombre, precio_usd, cantidad, porcentaje")
    ...
    if (rows.length === 0) {
      revalidatePath("/")
      return { ok: true, data: { updated: 0, products: [] } }
    }
    ...
    const payload = rows.map((row) => {
      ...
      return {
        ...
        precio_usd: base.precio_usd,
        cantidad: base.cantidad,
        porcentaje: base.porcentaje,
        tasa: nuevaTasa,
        ...calculateDerived(base),
      }
    })

    // Upsert por lotes: requiere que products.id sea 'generated BY DEFAULT as identity'.
    // Se pide .select() en cada lote para devolver al cliente las filas ya actualizadas
    // y que la UI se refresque sin depender de un reload manual.
    ...
    for (let i = 0; i < payload.length; i += UPSERT_CHUNK_SIZE) {
      ...
      updatedRows.push(...((upsertData ?? []) as Record<string, unknown>[]))
    }

    const products = updatedRows.map(mapProductRow).sort((a, b) => a.id - b.id)

    revalidatePath("/")
    return { ok: true, data: { updated: payload.length, products } }
  } catch (error) {
